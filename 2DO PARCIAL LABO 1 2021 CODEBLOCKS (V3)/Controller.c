...
/*int controller_obtenerEditorial(LinkedList* pListaLibros, LinkedList* pListaEdit, char* descEdit)
{
    int retorno = 0;
    int i;
    int j;
    ...
    if(pListaLibros != NULL && pListaEdit != NULL)
    {
        for(i=0; i<lenL; i++)
        {
            auxL = ll_get(pListaLibros, i);
            libro_getEditorialId(auxL, &idEL);
            for(j=0; j<lenE; j++)
            {
                auxE = ll_get(pListaEdit, j);
                edit_getId(auxE, &idE);
                if(idEL == idE)
                {
                    edit_getDesc(auxE, descEdit);
